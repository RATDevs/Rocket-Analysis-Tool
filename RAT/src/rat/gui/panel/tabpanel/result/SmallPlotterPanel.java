...
public class SmallPlotterPanel extends JPanel {
	...
	private static final int SMALL_WINDOW_SIZE_X = 250;
	private static final int SMALL_WINDOW_SIZE_Y = 150;
	private static final int SPACE_X = 50;
	private static final int SPACE_Y = 25;

	public SmallPlotterPanel(String x_name, String y_name, double[] x_val,
			double[] y_val) {
		...
		if (size_x_max != size_x_min)
			factor_x = (SMALL_WINDOW_SIZE_X - SPACE_X)
					/ (Math.abs(size_x_max) + Math.abs(size_x_min));
		else
			factor_x = 1;
		if (size_y_max != size_y_min)
			factor_y = (SMALL_WINDOW_SIZE_Y - SPACE_Y)
	...
	public Dimension getMaximumSize() {
		return new Dimension(SMALL_WINDOW_SIZE_X, SMALL_WINDOW_SIZE_Y);
	}
	...
	public static void main(String[] args) {
		...
		f.add(new SmallPlotterPanel("x-Achse", "y-Achse", x_values, y_values));
		f.setVisible(true);
		f.pack();
	}
}
